...
app.patch("/api/users/:id", requireAdmin, async (req, res) => {
  try {
    ...
    if (req.body?.password != null) {
      ...
      update.must_reset_password = p === "123456";
    }
...
app.get("/api/user/notifications", requireUser, blockIfMustReset, async (req, res) => {
  try {
    const q = (req.query.q || "").toString().trim();
    const filterRead = (req.query.filter || "").toString().trim();
...
function requireEnv(name) {
  ...
  if (!v) {
    console.error(`Faltou ${name} nas variáveis de ambiente.`);
    process.exit(1);
  }
  return v;
}

// ✅ Start: sobe HTTP primeiro (Render detecta porta), depois tenta DB/seed com retry
(async () => {
  requireEnv("JWT_SECRET");

  const port = Number(process.env.PORT || 10000);

  // Render precisa bind em 0.0.0.0
  app.listen(port, "0.0.0.0", () => console.log(`Server on :${port}`));

  // tenta DB/seed em background, sem derrubar o processo
  const attempt = async () => {
    try {
      await connectDB();
      await seedAdminAndSamples();
      console.log("[boot] DB + seed ok");
    } catch (e) {
      console.error("[boot] DB/seed falhou:", e?.message || e);
